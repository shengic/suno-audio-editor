...
- **Window auto-fits current content.** `_resize_to_fit()` reads `winfo_reqheight()` after `update_idletasks()` and sets the window height to match — width is preserved. Called on init (deferred via `after(0)` so widgets have their real requested sizes), and on lyrics panel show/hide. `main.py`'s `820x540` is just an initial hint; the auto-fit takes over as soon as the mainloop runs.
