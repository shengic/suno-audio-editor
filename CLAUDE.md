...
- `app.py` — `App`, the composition root. Builds the import panel (URL entry + drop zone + cover art label + title), region-info panel, status banner, waveform card (waveform + zoom/volume sliders), and transport bar (▶ 播放 / ⏸ 暫停 / ⏹ 停止 / 試聽選區 / ☑ 選區循環播放 / 儲存為 MP3 / 離開). Owns the single `AudioPlayer` and `WaveformView` instances; binds global keyboard shortcuts; registers global DND targets (root + drop_zone + url_entry so drops work anywhere in the UI); runs the `_tick()` poll loop. State is plain attributes and `tk.Variable`s bound directly to widgets — there is intentionally no separate state/store module; Tk's own `Variable` mechanism is the only reactive layer used.
