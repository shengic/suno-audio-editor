...
- **SRT / LRC lyric-file support.** Drop a `.srt` or `.lrc` file anywhere in the window (in any order relative to the audio). The lyrics panel is grid_remove'd until a file is loaded — no empty placeholder. Lines whose start time falls in the current selection region are shown; during playback, the current line is highlighted (LRC end-time inferred from the next line's start). Window height grows from 540 to 640 the first time the panel is shown, and never shrinks back automatically (respects user resize).
